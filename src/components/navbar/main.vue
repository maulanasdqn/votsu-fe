...
<template>
  <nav
    class="sticky top-0 dark:bg-gray-800 bg-yellow-200 flex w-full h-auto py-4 justify-between transition-all ease-in-out font-sans"
  >
    <div class="flex px-4 gap-x-4 items-center">
      <h1 class="dark:text-white text-gray-600 font-bold">
        {{ isDark ? "V O T S U" : "V O T S U" }}
      </h1>
      <svg v-if="isDark" class="w-6 text-white" viewBox="0 0 1000 1000">
        <path
        ...
        /></svg
      ><svg v-else class="w-6 text-white" viewBox="0 0 1000 1000">
        <path
      ...
      >
        <span class="text-1xl">Home</span>
        <span class="text-1xl">Informasi</span>
        <span class="text-1xl">Data Kandidat</span>
      </div>
    </div>
    ...
    <div class="flex justify-beetween gap-x-4 items-center px-6 cursor-pointer">
      ...
      >
        ...
        <router-link
          to="/register"
          class="text-1xl font-bold flex items-center justify-center border-2 dark:bg-gray-800 bg-gray-700 border-yellow-200 dark:rounded-none rounded-lg dark:border-none p-2 text-yellow-100 dark:text-white"
          >Register</router-link
        >
      </div>
      <div
        @click="toggleTheme()"
        class="flex h-5 w-10 rounded-xl transition-all justify-between dark:bg-white ease-in ease-out bg-yellow-100 border-1 border-yellow-300 dark:border-gray-400"
      >
        <svg
          v-if="isDark"
        ...
        <div
          class="flex transition-all order-2 dark:order-1 justify-center items-center rounded-full text-white dark:text-gray-500 dark:bg-blue-200 bg-yellow-500 w-5.5"
        ></div>
      </div>
    </div>
  </nav>
